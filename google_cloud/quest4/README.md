# Quest 4 - GSP323 
## Execute no cloudshell acrescentendo os parametros do seu desafio
### Task 1: Run a simple Dataflow job
Tabela de saida **BigQuery output table** da Task1
```cmd
REGION=
...
```
### Acesse o Menu Google Cloud
#### API & Credintials > Create Credintials > API KEY > Clique no botao para copiar a chave
##### Abra e execute em outro Cloud Shell colando a chave da API e cole na variavel de ambiente a seguir
```cmd
API_KEY=
```
##### Copie da Task3  para a variavelde ambiente a seguir
```cmd
Bucket_TASK_3=
```
##### Copie da Task4 para a variavelde ambiente a seguir
```cmd
Bucket_TASK_4=
```
##### Execute o próximo script e pressione Y para confirmar, clique no link que aparece pra copiar a chave e cole novamente no Shell.
```cmd
...
```
### Procure no Cloud Menu  ```Dataproc``` > Clique em cluster-b53a
>Vm Instance > ssh > ```hdfs dfs -cp gs://cloud-training/gsp323/data.txt /data.txt```
### No painela esquerdaselecione jobs > Submit job
