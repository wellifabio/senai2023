...
import { StyleSheet, Text, View, TextInput, Button, Image, FlatList, AsyncStorage } from 'react-native';
...
export default function App() {
  ...
  useEffect(() => {
    async function carregarResultados() {
      try {
        const resultadosString = await AsyncStorage.getItem('resultados');
        if (resultadosString !== null) {
          setResultados(JSON.parse(resultadosString));
        }
      } catch (error) {
        console.error(error);
      }
  ...
  const calcular = async () => {
    if (distancia > 0 && corrente > 0) {
      ...
      let novosResultados = [...resultados, resultado]
      try {
        await AsyncStorage.setItem('resultados', JSON.stringify(novosResultados));
